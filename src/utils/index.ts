...
export const encryptionStr = (data: string) => {
  let urlInCodeStr = encodeURI(data.toString());
  let rst = "";
  for (let index = 0; index < urlInCodeStr.length; index++) {
    const element = urlInCodeStr[index];
    let temp = (element.charCodeAt(0) ^ 188).toString(16);
    rst = `${rst}${temp}`;
  }
  return rst;
}
